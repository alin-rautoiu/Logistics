class Pawn extends Entity {
    ...
    public get movementTarget() {
        ...
        return currentTask?.movementTarget;
    }

    public set movementTarget(target: MovementTarget) {
        const currentTask = this.getCurrentTask();
        if (target) {
    ...
    constructor(sketch: any, x = 400, y = 200, diameter = 18, speed: number, searchRadius: number, pg: any, target: any, idx: number, shares?: boolean) {
    ...
    receivePotentialLocation(target: Entity) {
        ...
        if (target instanceof Goal) {
            ...
            if (this.knownLocations.indexOf(target) >= 0) return target;
    ...
    findInRange(l: Vector, range: number = 10): boolean {
