// Democracy Canvas 2 — Tutorialize: proportional representation.
// 60 % of the group moves toward the top-voted resource; the remaining 40 % toward the runner-up.
// Shows how a proportional system allows minority preferences to be partially served.
class DemocracyCanvas2 extends DemocracyCanvasBase {
    ...
    draw() {
        ...
        if (frame === 1) {
            ...
            if (winner) {
                const alive = this.pawns.filter(p => (p as any).behavior !== 'dead');
